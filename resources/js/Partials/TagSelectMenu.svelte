...
<Modal bind:showModal on:canceled={reset} title={title} showFooterMenuOnMobile={false}>
    ...
    <div class="relative">
        <input on:input={(e) => search(e.target.value)} bind:this={searchInput} type="text" placeholder="Search tags..."
               class="px-10 w-full border-0 border-b border-gray-100 peer focus:border-primary-200 focus:outline-none focus:ring-0 dark:bg-gray-900 dark:text-gray-50 dark:border-gray-700 dark:focus:border-primary-400"/>

        <div class="absolute inset-y-0 left-2 flex items-center text-gray-300 peer-focus:text-primary-600 dark:text-gray-500 dark:peer-focus:text-primary-200">
            ...
            {#each filteredTags as tag (tag.id)}
                {#if getIndexOfTagId(tag.id, internalSelectedTags) !== -1}
                    <button on:click|stopPropagation={() => selectTag(tag)} type="button" title={tag.name}
                            class="flex items-center py-1.5 px-3 border border-primary-100 bg-primary-50 rounded-full hover:border-primary-200 focus:outline-none dark:bg-primary-500 dark:border-primary-300 dark:hover:border-primary-200">
                        <span class="text-sm text-primary-900 truncate dark:text-white">{tag.name}</span>
                    </button>
                {:else}
                    <button on:click|stopPropagation={() => selectTag(tag)} type="button" title={tag.name}
                            class="flex items-center py-1.5 px-3 border border-gray-100 bg-gray-50 rounded-full hover:border-gray-200 focus:outline-none dark:bg-gray-800 dark:border-gray-600 dark:hover:border-gray-400">
        ...
        <div class="absolute bottom-0 left-1/2">
            {#if internalSelectedTags.length}
                <button on:click={() => {internalSelectedTags = []; showUntagged = false}} type="button"
                        class="absolute bottom-0 left-0 flex items-center w-max text-gray-600 -translate-x-1/2 group hover:text-gray-700 dark:text-gray-300 dark:hover:text-gray-200">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"
                         class="size-5 fill-gray-400 group-hover:fill-gray-500 dark:fill-gray-500 dark:group-hover:fill-gray-400">
                        <path fill-rule="evenodd"
